  ...
  return (
    <div className="flex gap-4 p-3 rounded-lg shadow-sm hover:shadow-md transition-shadow bg-white">
      <img
        src={item.imageURL}
        alt={item.name}
        className="w-16 h-16 object-cover rounded-md border hidden md:flex"
      />

      <div className="flex-1 flex flex-col justify-between">
        <div className="font-semibold text-gray-800 truncate w-max-[150px] md:w-max-[180px]">{item.name}</div>
      ...
      <div className="relative flex flex-col justify-between items-end ">
        ...
        <div className="items-center flex gap-1 mt-2">
          <Minus size={14} onClick={() => decreaseQuantity(item.uniqueId!)} className="hover:cursor-pointer" />
          <span className="px-2 text-sm font-medium">{item.quantity}</span>
          <Plus size={14} onClick={() => increaseQuantity(item.uniqueId!)} className="hover:cursor-pointer" />
        </div>
      </div>
    </div>
  )
